...
"
" ========== BimbaLaszlo (.github.io|gmail.com) ========== 2015.03.11 11:12 ==
...
function! s:GetLimits( first, last, mode )
    ...
    unlet label
    for label in g:GCode_labels
        let limits[ label ][ 'diff' ] = abs( limits[ label ][ 'min' ] - limits[ label ][ 'max' ] )
    endfor
    ...
    for name in g:GCode_labels
        ...
        for key in [ 'min', 'max', 'min_abs', 'diff' ]
